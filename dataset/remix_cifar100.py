...
def get_cifar100(root, l_samples, u_samples, transform_train=transform_train, transform_strong=transform_strong,
    ...
    train_labeled_dataset = CIFAR100_labeled(root, train_labeled_idxs, train=True, transform=TransformDoub(transform_strong, transform_train))
...
def train_split(labels, n_labeled_per_class, n_unlabeled_per_class,rand_number):
    ...
    for i in range(100):
        ...
        if rand_number!=0:
            np.random.shuffle(idxs)#Since we brought the experimental results on CIFAR 100 presented in the CoSSL paper, we shuffle idxs for CIFAR100 as like CoSSL
