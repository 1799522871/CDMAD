...
parser.add_argument('--train-dir', default='D:\\codestudy\\MyPythonProject\\remote-sesing\\imbalanced classification\\CDMAD\\data\\PatternNet_split_TL\\train',
                    type=str, help='train dataset path')
parser.add_argument('--test-dir', default='D:\\codestudy\\MyPythonProject\\remote-sesing\\imbalanced classification\\CDMAD\\data\\PatternNet_split_TL\\test',
                    type=str, help='test dataset path')
# Checkpoints
parser.add_argument('--resume', default='', type=str, metavar='PATH',
                    help='path to latest checkpoint (default: none)')
parser.add_argument('--out', default='result1',
                        help='Directory to output the result1')
# Miscs
...
if args.dataset=='cifar10':
    ...
    import dataset.UCM_Dataset as dataset
    print(f'==> Preparing imbalanced PatternNet')
    num_class = 38
...
def main():
    ...
    if args.dataset == 'cifar10':
        ...
        train_labeled_set, train_unlabeled_set, test_set = dataset.get_UCM(args.train_dir, args.test_dir, N_SAMPLES_PER_CLASS, U_SAMPLES_PER_CLASS, rand_number=args.manualSeed)

    labeled_trainloader = data.DataLoader(train_labeled_set, batch_size=args.batch_size, shuffle=True, num_workers=0,
                                          drop_last=True) # 原先num_workers=4
    unlabeled_trainloader = data.DataLoader(train_unlabeled_set, batch_size=int(args.unlabeledratio*args.batch_size), shuffle=True, num_workers=0,drop_last=True)
...
def train(labeled_trainloader,unlabeled_trainloader, model,optimizer, ema_optimizer, criterion, epoch):
    ...
    for batch_idx in range(args.val_iteration):
        ...
        # print(targets_x.dtype) # torch.int32
...
def validate(valloader,model,criterion,mode):
    ...
    with torch.no_grad():
        ...
        # white = torch.ones(1,3, 32, 32).cuda()
        white = torch.ones(1,3, 256, 256).cuda()
        biaseddegree, _ = model(white)
